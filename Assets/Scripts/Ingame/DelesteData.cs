...
namespace TempestWave.Ingame
{
    ...
    public class DelesteBlockData
    {
        ...
        public void ParseBlock(GameManager Game, ref int ID, ref double Time, ref DelesteGlobalData StaticData)
        {
            int MaxBit = 1;
            ...
            for(int i = 0; i < MaxBit; i++)
            {
                ...
                {
                    Game.Dispensor.CreateNote(ID++, 0, new Color32(255, 255, 255, 0), NoteInfo.SystemNoteScroller, FlickMode.None, (float)Time * 60, (float)StaticData.Scroll[0][0], 0, 0, new List<int>());
                ...
                {
                    Game.Dispensor.CreateNote(ID++, 0, new Color32(255, 255, 255, 0), NoteInfo.SystemNoteScroller, FlickMode.None, (float)Time * 60, 1, 0, 0, new List<int>());
                ...
                for (int j = 0; j < Datas.Count; j++)
                {
                    ...
                    if (Datas[j].Length >= 3)
                    {
                        ...
                        else
                        {
                            ...
                            else { throw new Exception("Error in start point parsing. Please check your beatmap."); }
                        }
                        StartIndex[j]++;
                    ...
                    if(Datas[j].Length >= 4)
                    {
                        ...
                        else
                        {
                            ...
                            else { throw new Exception("Error in end point parsing. Please check your beatmap."); }
                        }
                        EndIndex[j]++;
                    }
                    ...
                    if (Mode.Equals(NoteInfo.NormalNote) && StaticData.HoldPrev[End] > 0)
                    {
                        Mode = NoteInfo.LongNoteEnd;
                        Prevs.Add(StaticData.HoldPrev[End]);
                        StaticData.HoldPrev[End] = 0;
                    }
                    else if(Mode.Equals(NoteInfo.NormalNote) && StaticData.TailPrev[Channel[j]] > 0)
                    {
                        Mode = NoteInfo.SlideNoteEnd;
                        Prevs.Add(StaticData.TailPrev[Channel[j]]);
                        StaticData.TailPrev[Channel[j]] = 0;
                    }
                    if(Mode.Equals(NoteInfo.SlideNoteStart) && StaticData.TailPrev[Channel[j]] > 0)
                    {
                    ...
                    if (!Flick.Equals(FlickMode.None) && StaticData.ConnectorPrev[Channel[j]].Count > 0)
                    {
                    ...
                    else if (Flick.Equals(FlickMode.None) && StaticData.ConnectorPrev[Channel[j]].Count > 0 && StaticData.BeforeTime[Channel[j]] != Time) { StaticData.ConnectorPrev[Channel[j]].Clear(); }

                    // 다음 노트와 연결될 노트에 대한 데이터를 전역 데이터에 입력합니다.
                    if (Mode.Equals(NoteInfo.LongNoteStart)) { StaticData.HoldPrev[End] = ID; }
                    else if (Mode.Equals(NoteInfo.SlideNoteStart) || Mode.Equals(NoteInfo.SlideNoteCheckpoint)) { StaticData.TailPrev[Channel[j]] = ID; }
                    if(!Flick.Equals(FlickMode.None))
                    {
                        if (Channel[j] % 4 == 0 || Channel[j] % 4 == 1)
                        {
                            if (Flick.Equals(FlickMode.Left)) { StaticData.ConnectorPrev[Channel[j]].Add(new double[3] { -1, End, ID }); }
                            else if (Flick.Equals(FlickMode.Right)) { StaticData.ConnectorPrev[Channel[j]].Add(new double[3] { 1, End, ID }); }
                        }
                    ...
                    Game.Dispensor.CreateNote(ID++, 1, new Color32(Color[j][0], Color[j][1], Color[j][2], Color[j][3]), Mode, Flick, (float)Time * 60, (float)(Speed[j] * StaticData.SpeedMultiplier), DataSender.ReturnRandWave() ? UnityEngine.Random.Range(1, 6) : (float)Start, (float)End, Prevs);
                }
